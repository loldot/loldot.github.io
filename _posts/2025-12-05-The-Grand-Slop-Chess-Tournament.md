...
A while ago, I watched Sebastian Lague's YouTube channel where he made a framework for writing a chess engine and challenged his viewers to make
...
I can only assume Gemini has never experienced such a failure in its corpus of Google's divine code, and hence was completely derailed
...
Claude Sonnet was seemingly unaffected by my test project blunder &mdash; it didn't need to run any tests. Claude blasted out a chess search function
with a number of advanced pruning techniques in a couple of seconds. When I tested it on my list of test positions, it found the best move in 9/14
...
Cline's agent mode. I let Codex submit its engine again, since it had actually produced something in the warmup round—perhaps giving it an
...
Like all the other bots, Claude implemented a principal variation search function with iterative deepening and a quiescence search.
...
Claude more or less implemented the [Simplified Evaluation Function][1] from Chess Programming Wiki.

In addition, Claude added aspiration windows, null-move pruning, late move reductions, and a quiescence search with delta pruning.
...
In this game against Gemini, Claude has allowed black to gain a pawn advantage with a tempo on white's bishop, but Stockfish actually
...
on how to protect its exposed bishop. Black's knight is a bit awkward, but Claude does not really have a much better position. Claude
...
queens and rooks. In the end, Gemini is up a knight in a 3v3 pawn endgame, which it easily wins.
...
to get in the game—unless of course black decides to help him develop and lose its bishop while he's at it. As white is
...
position where Grok has a forcing sequence to take white's bishop. Then Grok decides to give up its bishop for a pawn with
