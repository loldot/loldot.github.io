...
# Hacktivitycon: Template Shack 

The template shack challenge appears to be some sort of online shop for bootsrap themes. 

The site seems to be mostly static and placeholder code. The only thing of note in the source code for the page is a comment that says:
...
Looking a bit closer we can see that some of the pages give a rather peculiar 404 message that differs from the 404 page outside the admin panel. It seems to print the url we tried to reach back to us. The title of the challenge seems to hint that the page could be voulnerable to a SSTI attack. I tried to insert {{ 1 + 1 }} in the url and check what is written back to us. Turns out it is in fact vulnerable to an SSTI. I already knew it was some python app from the http header 'Server: Werkzeug/1.0.1 Python/3.6.9' we received from the server with every request. So enumerating to find the template engine use was just a matter of trying and failing with a few payloads. Turns out it is using the jinja templating engine, so we can find an appropriate payload to something appropriate for jinja RCE.
